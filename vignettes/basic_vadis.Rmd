...
**IMPORTANT:** Standardizing inputs in this way is essential for reliable calculation of the similarity coefficients in line 2 (Step 4). See `?stand` for details. However, **only fixed effects predictors** should be standardized in this way. The `all.vars()` function is a quick way to convert a formula into a vector of names, but it's necessary to specify the columns that should *not* be standardized. 
