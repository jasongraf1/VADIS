...
We chose ±1 as the value for our hypothetical constraints since a change from -1 to 1 on the log odds scale represents an increase in probability of approximately 50 percentage points: *P* = logit^-1^(-1) = 0.27; *P* = logit^-1^(1) = 0.73. It seems very unlikely that we'd ever see such a big reversal in *every* constraint's effect between two related varieties. Note that these values assume that inputs are standardized according to the procedure described in @Gelman2008. 
