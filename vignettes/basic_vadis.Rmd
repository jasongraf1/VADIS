...
Next we simply loop over the datasets in `data_list` and fit a model to each one. Before fitting we'll center and rescale our model inputs [see @Gelman2008] with the `stand()` function. 

**Note:** Standardizing inputs in this way is essential for reliable calculation of the similarity coefficients in line 2 (Step 4). See `?stand` for details.
...
One way we can visualize distances is through clustering methods. There are many different functions and packages for doing this. The simplest case is probably the `hclust()` function.
...
We can look at the fused matrix combining the 3 lines of evidence.
